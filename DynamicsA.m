function M = DynamicsA(N,t,ba,a,k)
%Function to simulate the model in the bounded rectangular region with 
%N particles, alignment strength a, and box dimensions scaled by k. On each 
%time step of the simulation the position and velocity of each particle is 
%recorded and at the end of the simulation this data is returned from the 
%simulation as a cell array M={X,Y,Vx,Vy} where X is the x coordinates, Y is the y 
...
xL=k*210; %Region length
yL=k*120; %Region height
%t=simulation time
...
P=zeros(N,4); 
for i=1:N 
    P(i,1)=xL/4+0.5*(0.5-rand)*xL/4; % Initiates x-coordinate.
    P(i,2)=yL/4+0.5*(0.5-rand)*yL/4; % Initiates y-coordinate.
...
for k=1:t %Simulate for t time steps
        ...
        for i=1:N % Go through every particle.
            ...
            CMAL=nh2DGRA(i,P,R,R,ba); %Call neighborhood and interaction calculator function
            C=CMAL(1,:); %Direction toward the LCM
            F=CMAL(2,:); %Local repulsive force  
            A=CMAL(2,:); %Local Alignment force
            ...
            Dir=d*D+c*C+rr*F+e*E+a*A; %Heading update formula ALIGNMENT ADDED
